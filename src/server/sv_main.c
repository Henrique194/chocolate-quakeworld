...
void SVC_Status(void) {
    ...
    for (i = 0; i < MAX_CLIENTS; i++) {
        ...
        if ((cl->state == cs_connected || cl->state == cs_spawned)
            ...
            ping = SV_CalcPing(cl);
            Con_Printf(
                "%i %i %i %i \"%s\" \"%s\" %i %i\n", cl->userid, cl->old_frags,
                (int) (realtime - cl->connection_started) / 60, ping, cl->name,
...
This lets you easily set up a private game, or a game that only allows
...
void SV_InitLocal(void) {
    ...
    Info_SetValueForStarKey(svs.info, "*version",
        va("%4.2f", VERSION), MAX_SERVERINFO_STRING);
