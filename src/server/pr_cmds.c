...
void PF_findradius(void) {
    ...
    for (i = 1; i < sv.num_edicts; i++, ent = NEXT_EDICT(ent)) {
        ...
        for (j = 0; j < 3; j++)
            eorg[j] =
                org[j]
                - (ent->v.origin[j] + (ent->v.mins[j] + ent->v.maxs[j]) * 0.5);
        if (Length(eorg) > rad)
            continue;
...
sizebuf_t* WriteDest(void) {
    ...
    switch (dest) {
        ...
        case MSG_INIT:
            if (sv.state != ss_loading)
                PR_RunError("PF_Write_*: MSG_INIT can only be written in spawn "
                            "functions");
            return &sv.signon;
...
void PF_infokey(void) {
    ...
    if (e1 == 0) {
        ...
        if (!strcmp(key, "ip"))
            value = strcpy(ov, NET_BaseAdrToString(
                                   svs.clients[e1 - 1].netchan.remote_address));
