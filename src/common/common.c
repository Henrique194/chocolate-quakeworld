...
config files) will be saved to. This can be overridden with the "-game"
command line parameter. The game directory can never be changed while
quake is executing. This is a precacution against having a malicious
...
//
static qboolean COM_SearchPak(const char* filename, pack_t* pak, FILE** file) {
...
//
static qboolean COM_SearchPaths(const char* filename, FILE** file) {
    const searchpath_t* search = com_searchpaths;
    for (; search; search = search->next) {
        // Is the element a pak file?
        if (search->pack) {
            if (COM_SearchPak(filename, search->pack, file)) {
                return true;
            }
            continue;
        }
        if (COM_SearchDir(search, filename, file)) {
            return true;
        }
    }
    return false;
...
int COM_FOpenFile(char* filename, FILE** file) {
    file_from_pak = 0;
    if (!COM_SearchPaths(filename, file)) {
...
int COM_FindMusicTrack(const char* track_file, FILE** file) {
    ...
    for (; search; search = search->next) {
        ...
        if (COM_SearchDir(search, track_file, file)) {
...
size_t FS_fread(void* ptr, size_t size, size_t nmemb, fshandle_t* fh) {
    ...
    long byte_size = nmemb * size;
    if (byte_size > fh->length - fh->pos) {
        // Just read to end.
        byte_size = fh->length - fh->pos;
    }
    long bytes_read = fread(ptr, 1, byte_size, fh->file);
    fh->pos += bytes_read;
...
int FS_feof(const fshandle_t* fh) {
    if (!fh) {
        errno = EBADF;
        return -1;
    }
