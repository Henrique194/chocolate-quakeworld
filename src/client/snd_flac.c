...
static void FLAC_Error_f(const FLAC__StreamDecoder* decoder,
...
FLAC_Read_f(const FLAC__StreamDecoder* decoder, FLAC__byte buffer[],
...
FLAC_Seek_f(const FLAC__StreamDecoder* decoder,
...
FLAC_Tell_f(const FLAC__StreamDecoder* decoder,
            FLAC__uint64* absolute_byte_offset, void* client_data) {
    const flacfile_t* ff = client_data;
...
FLAC_Length_f(const FLAC__StreamDecoder* decoder, FLAC__uint64* stream_length,
              void* client_data) {
    const flacfile_t* ff = client_data;
...
static FLAC__bool FLAC_EOF_f(const FLAC__StreamDecoder* decoder,
                             void* client_data) {
    const flacfile_t* ff = client_data;
...
FLAC_Write_f(const FLAC__StreamDecoder* decoder, const FLAC__Frame* frame,
...
static void FLAC_Meta_f(const FLAC__StreamDecoder* decoder,
...
static qboolean S_FLAC_CodecOpenStream(snd_stream_t* stream) {
    ...
    int rc = FLAC__stream_decoder_init_stream(
        ff->decoder, FLAC_Read_f, FLAC_Seek_f, FLAC_Tell_f, FLAC_Length_f,
        FLAC_EOF_f, FLAC_Write_f, FLAC_Meta_f, FLAC_Error_f, ff);
...
snd_codec_t flac_codec = {.type = CODECTYPE_FLAC,
