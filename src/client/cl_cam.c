...
qboolean Cam_DrawPlayer(int playernum) {
    return !cl.spectator || !autocam || !locked || cl_chasecam.value == 0
...
static float Cam_TryFlyby(player_state_t* self, player_state_t* player,
...
static qboolean InitFlyby(player_state_t* self, player_state_t* player,
                          int checkvis) {
    float f, max;
    vec3_t vec, vec2;
